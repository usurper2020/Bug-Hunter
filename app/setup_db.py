        ...
        config_template = "config.template.json"
            ...
            if os.path.exists(config_file):
                ...
                config = json.load(f)
                         ...
                         "user": args.user
                         or config.get("database", {}).get("user", "bughunter_user"),
                         "password": args.password
                         or config.get("database", {}).get("password", "your-secure-password-here"),
                         "pool_size": 5,
                         "max_overflow": 10,
                         "pool_timeout": 30,
                         "pool_recycle": 3600,
                                    ...
                                    Main entry point for database setup utility.
                                        ...
                                        4. Provides next steps for application deployment
                                            ...
                                            --password: Database password
                                                ...
                                                description="Set up Bug Hunter database")

                                                # Database connection options
                                                parser.add_argument(
                                                "--host", help="Database host (default: localhost)")
                                                parser.add_argument(
                                                "--port", type=int, help="Database port (default: 5432)")
                                                parser.add_argument(
                                                "--name", help="Database name (default: bughunter_db)")
                                                parser.add_argument(
                                                "--user", help="Database user (default: bughunter_user)")
                                                parser.add_argument(
                                         ...
                                         print(
                                         "2. Update config.json with your database credentials if needed")
                                         print(
                                         "3. Run the application with: python run_integrated.py")
                                                else:
                                                sys.exit(1)
