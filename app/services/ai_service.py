...
class AIService:
    ...
    def _initialize_api_client(self):
        if self.api_type == 'openai':
            ...
            if not (openai_api_key := self.config.get('openai_api_key') or os.getenv('OPENAI_API_KEY')):
                raise ValueError("OpenAI API key not configured")
            else:
    ...
    def _load_models_configuration(self):
    ...
    def _prepare_analysis_request(self, scan_result: Dict[str, Any]) -> AIAnalysisRequest:
    ...
    def _create_analysis_result(self, analysis: Dict[str, Any], start_time: datetime) -> AIAnalysisResult:
    ...
    async def _get_codegpt_response(self, prompt: str) -> str:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post('https://api.codegpt.co/v1/chat/completions', headers=headers, json=data) as response:
                if response.status != 200:
                    result = await response.json()
                    raise RuntimeError(f"CodeGPT API error: {result.get('error', {}).get('message', 'Unknown error')}")
                result = await response.json()
    ...
    def _analyze_vulnerabilities(self, url: str, content: str, soup: BeautifulSoup, technologies: List[str]) -> List[Dict]:
        vulnerabilities = []
        checks = [
            {
    ...
    def _generate_recommendations(self, technologies: List[str], vulnerabilities: List[Dict]) -> List[str]:
        recommendations = []
